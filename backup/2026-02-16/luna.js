...
(function () {
    ...
    ============================================================ */
    class VideoScrubber {
        constructor(canvasId, trackId, dir, frameCount = 192) {
        ...
        async preload(onProgress) {
            ...
            for (let s = 0; s < this.totalFrames; s += BATCH) {
                ...
                const batch = [];
                for (let i = s; i < end; i++) {
                    batch.push(this.loadOne(i).then(r => {
                        this.frames.push(r);
                        loaded++;
        ...
        onScroll(st) {
            ...
            this.targetIdx = p >= this.PLAY_END
                ? this.frames.length - 1
                : Math.min(Math.floor((p / this.PLAY_END) * this.frames.length), this.frames.length - 1);
    ...
    function handleScroll() {
        ...
        // Intro scrubber
        if (introScrubber) {
            const p = introScrubber.onScroll(st);
        ...
        // Bridge scrubber
        if (bridgeScrubber) {
            const p = bridgeScrubber.onScroll(st);
    ...
    function tick() {
        /* scrubbers */
        if (introScrubber) introScrubber.update();
        if (bridgeScrubber) bridgeScrubber.update();
    ...
    async function boot() {
        ...
        introScrubber = new VideoScrubber('lunaCanvas', 'scrollTrack', 'luna-frames/');
        bridgeScrubber = new VideoScrubber('lunaCanvas2', 'bridgeTrack', 'luna-frames2/');

        /* Preload both */
        const total = introScrubber.totalFrames + bridgeScrubber.totalFrames;
        let globalLoaded = 0;

        const updateGlobalLoader = (n) => {
            globalLoaded++;
            const pct = Math.round((globalLoaded / total) * 100);
            if (loaderFill) loaderFill.style.width = pct + '%';
            if (loaderPct) loaderPct.textContent = pct + '%';
        };

        await Promise.all([
            introScrubber.preload(updateGlobalLoader),
            bridgeScrubber.preload(updateGlobalLoader)
        ]);
